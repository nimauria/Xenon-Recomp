...
The guest-networking surfaces in Xenia and ReXGlue were also cross-checked.
Their `NetDll_*`, XNet and host-socket implementations describe the separate
Xbox compatibility/data-plane boundary; they are not a hosted service protocol
and are not copied into this control-plane client. UnleashedRecomp exposes
title-local socket/XNet stubs and an unrelated updater client, while the other
reviewed recompilation projects likewise do not provide a reusable secure
online-services control plane. Xenon therefore keeps guest socket translation,
the Xenon Network control plane, and future gameplay transport as three explicit
responsibilities.
...
- Qt performs certificate-chain and hostname validation, explicitly requires
  TLS 1.2 or later, enables in-memory HSTS handling, and never calls
  `ignoreSslErrors`.
- API redirects are not followed. This prevents cross-origin bearer leakage,
  redirect-based endpoint-policy bypass, and redirect-induced method changes.
- Cookies and response caching are disabled for the service transport. Only a
  small allowlist of non-secret response headers is retained.
- Plaintext is restricted to explicit loopback Development endpoints.
- Bootstrap-discovered realtime and relay endpoints must remain on the base
  service origin unless an alternate origin was explicitly configured first.
- Endpoint URLs use a strict ASCII/canonical subset: credentials, queries,
  fragments, backslashes, invalid ports/hosts, dot segments and encoded path
  separators are rejected.
- Connect, request, and realtime heartbeat timeouts are bounded.
- Request/response bodies, headers, request count, queued-event count,
  capabilities, identifiers and realtime events have configured or protocol
  bounds.
- JSON parsing rejects invalid number grammar, invalid escapes, unpaired UTF-16
  surrogates, duplicate keys, raw control characters and excessive nesting.
- The central route catalogue—not individual callers—enforces which operations
  require authentication. Invalid/expired bearer responses clear the in-memory
  session and move authentication state to `Expired`.
...
- Memory-only credentials are validated before use and overwritten on
  replacement, rejection, shutdown and destruction on a best-effort basis.
