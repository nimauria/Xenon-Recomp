...
bool write_msaa_sample(VkPhysicalDevice physical_device, VkDevice device,
                       CommandQueue& queue, VkImage target,
                       VkImageLayout target_layout, VkImageView target_view,
                       VkFormat format, MsaaSamples samples,
                       std::uint32_t host_sample, std::uint32_t left,
                       std::uint32_t top, std::uint32_t width,
                       std::uint32_t height, std::uint32_t bytes_per_pixel,
  ...
  options.format = ShaderBinaryFormat::Spirv;
  const auto vs = compiler.compile(make_transfer_fullscreen_vertex_shader(), options);
  const auto ps = compiler.compile(make_color_sample_write_shader(), options);
  if (!vs.succeeded || !ps.succeeded) {
    error = !vs.succeeded && !vs.diagnostics.empty() ? vs.diagnostics.front()
            : !ps.diagnostics.empty() ? ps.diagnostics.front()
                                      : "Vulkan sample upload shader compilation failed";
    return false;
  }

  VkImage upload_image = VK_NULL_HANDLE;
  VkDeviceMemory upload_memory = VK_NULL_HANDLE;
  VkImageView upload_view = VK_NULL_HANDLE;
  VkDescriptorSetLayout set_layout = VK_NULL_HANDLE;
  VkPipelineLayout pipeline_layout = VK_NULL_HANDLE;
  VkDescriptorPool descriptor_pool = VK_NULL_HANDLE;
  VkShaderModule vs_module = VK_NULL_HANDLE;
  ...
  auto cleanup = [&] {
    ...
    if (pipeline_layout) vkDestroyPipelineLayout(device, pipeline_layout, nullptr);
    if (set_layout) vkDestroyDescriptorSetLayout(device, set_layout, nullptr);
    if (upload_view) vkDestroyImageView(device, upload_view, nullptr);
    if (upload_image) vkDestroyImage(device, upload_image, nullptr);
    if (upload_memory) vkFreeMemory(device, upload_memory, nullptr);
  };

  Buffer staging;
  if (!staging.initialize(physical_device, device,
                          VkDeviceSize(row_pitch) * height,
                          VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                          VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                              VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)) {
    error = staging.error(); return false;
  }
  std::span<std::byte> mapped;
  if (!staging.map(mapped)) { error = staging.error(); return false; }
  std::memcpy(mapped.data(), source.data(), std::size_t(row_pitch) * height);
  staging.unmap();
  ...
  if (vkCreateImage(device, &image_info, nullptr, &upload_image) != VK_SUCCESS) {
    error = "Vulkan sample upload image creation failed"; cleanup(); return false;
  }
  VkMemoryRequirements requirements{};
  vkGetImageMemoryRequirements(device, upload_image, &requirements);
  VkMemoryAllocateInfo allocation{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
  allocation.allocationSize = requirements.size;
  allocation.memoryTypeIndex = find_memory_type(
      physical_device, requirements.memoryTypeBits,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
  if (allocation.memoryTypeIndex == UINT32_MAX ||
      vkAllocateMemory(device, &allocation, nullptr, &upload_memory) != VK_SUCCESS ||
      vkBindImageMemory(device, upload_image, upload_memory, 0) != VK_SUCCESS) {
    error = "Vulkan sample upload image allocation failed"; cleanup(); return false;
  }
  VkImageViewCreateInfo view_info{VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO};
  view_info.image = upload_image;
  view_info.viewType = VK_IMAGE_VIEW_TYPE_2D;
  view_info.format = format;
  view_info.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1};
  if (vkCreateImageView(device, &view_info, nullptr, &upload_view) != VK_SUCCESS) {
    error = "Vulkan sample upload view creation failed"; cleanup(); return false;
  }
  ...
  set_info.pBindings = bindings.data();
  if (vkCreateDescriptorSetLayout(device, &set_info, nullptr, &set_layout) != VK_SUCCESS) {
    error = "Vulkan sample upload descriptor layout failed"; cleanup(); return false;
  }
  VkPipelineLayoutCreateInfo layout_info{VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO};
  layout_info.setLayoutCount = 1; layout_info.pSetLayouts = &set_layout;
  if (vkCreatePipelineLayout(device, &layout_info, nullptr, &pipeline_layout) != VK_SUCCESS) {
    error = "Vulkan sample upload pipeline layout failed"; cleanup(); return false;
  }
  const std::array pool_sizes{
      VkDescriptorPoolSize{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1},
      VkDescriptorPoolSize{VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1}};
  ...
  pool_info.pPoolSizes = pool_sizes.data();
  if (vkCreateDescriptorPool(device, &pool_info, nullptr, &descriptor_pool) != VK_SUCCESS) {
    error = "Vulkan sample upload descriptor pool failed"; cleanup(); return false;
  }
  VkDescriptorSetAllocateInfo set_allocate{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO};
  set_allocate.descriptorPool = descriptor_pool; set_allocate.descriptorSetCount = 1;
  set_allocate.pSetLayouts = &set_layout;
  VkDescriptorSet descriptor_set = VK_NULL_HANDLE;
  if (vkAllocateDescriptorSets(device, &set_allocate, &descriptor_set) != VK_SUCCESS) {
    error = "Vulkan sample upload descriptor allocation failed"; cleanup(); return false;
  }
  VkDescriptorBufferInfo constant_info{constants.buffer(), 0, sizeof(TransferConstants)};
  VkDescriptorImageInfo source_info{};
  source_info.imageView = upload_view;
  source_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
  std::array<VkWriteDescriptorSet, 2> writes{};
  writes[0] = {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, descriptor_set, 0,
               0, 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, nullptr, &constant_info, nullptr};
  ...
  if (!create_module(vs, vs_module) || !create_module(ps, ps_module)) {
    error = "Vulkan sample upload shader module creation failed"; cleanup(); return false;
  }
  const std::array stages{
      VkPipelineShaderStageCreateInfo{VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
          nullptr, 0, VK_SHADER_STAGE_VERTEX_BIT, vs_module, "main", nullptr},
  ...
  VkPipelineInputAssemblyStateCreateInfo assembly{VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO};
  assembly.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
  const VkViewport viewport{float(left), float(top), float(width), float(height), 0, 1};
  const VkRect2D scissor{{static_cast<std::int32_t>(left), static_cast<std::int32_t>(top)},
                         {width, height}};
  VkPipelineViewportStateCreateInfo viewport_state{VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO};
  viewport_state.viewportCount=1; viewport_state.pViewports=&viewport;
  viewport_state.scissorCount=1; viewport_state.pScissors=&scissor;
  ...
  raster.polygonMode=VK_POLYGON_MODE_FILL; raster.cullMode=VK_CULL_MODE_NONE; raster.lineWidth=1;
  VkPipelineMultisampleStateCreateInfo multisample{VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO};
  multisample.rasterizationSamples=sample_count(samples);
  multisample.sampleShadingEnable=VK_TRUE; multisample.minSampleShading=1.0f;
  VkPipelineColorBlendAttachmentState attachment{}; attachment.colorWriteMask=0xF;
  VkPipelineColorBlendStateCreateInfo blend{VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO};
  blend.attachmentCount=1; blend.pAttachments=&attachment;
  VkPipelineRenderingCreateInfo rendering{VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO};
  ...
  if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipeline_info,
                                nullptr, &pipeline) != VK_SUCCESS) {
    error = "Vulkan sample upload pipeline creation failed"; cleanup(); return false;
  }
  ...
  const bool submitted = queue.execute([&](VkCommandBuffer command) {
    ...
    barriers[0].image=upload_image;
    barriers[0].subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};
    VkDependencyInfo dependency{VK_STRUCTURE_TYPE_DEPENDENCY_INFO};
    dependency.imageMemoryBarrierCount=1; dependency.pImageMemoryBarriers=barriers.data();
    vkCmdPipelineBarrier2(command,&dependency);
    VkBufferImageCopy copy{};
    copy.bufferRowLength=row_pitch/bytes_per_pixel;
    copy.imageSubresource={VK_IMAGE_ASPECT_COLOR_BIT,0,0,1};
    copy.imageExtent={width,height,1};
    vkCmdCopyBufferToImage(command,staging.buffer(),upload_image,
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,1,&copy);
    barriers[0].oldLayout=VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
    barriers[0].newLayout=VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    barriers[1]={VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2};
    barriers[1].srcStageMask=VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT;
    barriers[1].srcAccessMask=VK_ACCESS_2_MEMORY_READ_BIT|VK_ACCESS_2_MEMORY_WRITE_BIT;
    barriers[1].dstStageMask=VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT;
    barriers[1].dstAccessMask=VK_ACCESS_2_COLOR_ATTACHMENT_READ_BIT|VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT;
    ...
    VkRenderingInfo render{VK_STRUCTURE_TYPE_RENDERING_INFO};
    render.renderArea=scissor; render.layerCount=1;
    render.colorAttachmentCount=1; render.pColorAttachments=&color;
    vkCmdBeginRendering(command,&render);
    vkCmdBindPipeline(command,VK_PIPELINE_BIND_POINT_GRAPHICS,pipeline);
    vkCmdBindDescriptorSets(command,VK_PIPELINE_BIND_POINT_GRAPHICS,pipeline_layout,
...
bool RenderTargetImage::initialize(VkPhysicalDevice physical_device,
  ...
  if (surface.msaa == MsaaSamples::X2) {
    ...
    const auto query = vkGetPhysicalDeviceImageFormatProperties(
        ...
        0, &image_properties);
    const bool supports_native_2x = query == VK_SUCCESS &&
  ...
  image_info.samples = sample_count(host_msaa_);
  image_info.tiling = VK_IMAGE_TILING_OPTIMAL;
  image_info.usage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
                     VK_IMAGE_USAGE_SAMPLED_BIT |
                     VK_IMAGE_USAGE_TRANSFER_SRC_BIT |
...
bool RenderTargetImage::upload_sample(
  ...
  const auto mapping = map_guest_sample_to_host(
      surface_.msaa, guest_sample, host_msaa_ == MsaaSamples::X2);
  const auto width = right > left ? right - left : 0;
  const auto height = bottom > top ? bottom - top : 0;
  if (!mapping || !bytes_per_pixel_ || !width || !height ||
      right > mip_width_ || bottom > mip_height_ ||
      row_pitch < width * bytes_per_pixel_ ||
      row_pitch % bytes_per_pixel_ ||
      std::uint64_t(row_pitch) * height > source.size()) {
    error_ = "invalid Vulkan selected-sample upload";
    return false;
  }
...
#ifdef XENON_HAS_DXC
  if (!write_msaa_sample(physical_device_, device_, queue, image_, layout_, view_,
                         format_, host_msaa_, mapping->sample, left, top,
                         width, height, bytes_per_pixel_, source, row_pitch,
                         error_))
    return false;
  layout_ = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
  return true;
#else
  error_ = "Vulkan selected-sample upload requires DXC";
  return false;
#endif
}
