...
class Backend::Impl {
 public:
  ...
  bool make_edram_canonical() {
    if (!edram) {
      error = "Vulkan canonical EDRAM checkpoint has no bound EDRAM";
      return false;
    }
  ...
  if (const auto* draw = std::get_if<ir::DrawPacket>(&command)) {
    ...
    if (state.edram_mode == EdramMode::Copy) {
      ...
      if (!resolve_vertices || !resolve_vertices->valid ||
        ...
        impl_->error = "Vulkan Xenos resolve has no valid rectangle vertices";
        return;
      }
      ...
      if (!impl_->memory->copy_physical_range(resolve_vertex_base,
                                               resolve_vertex_snapshot)) {
        impl_->error = "Vulkan resolve vertex snapshot is outside physical memory";
        return;
      }
          ...
          if (selected_count == 4) {
            ...
            if (!source->readback_sample(
                ...
                !average_host_color_samples(
                    ...
                    all_average_pitch)) {
              impl_->error =
                  "Vulkan Xenos four-sample resolve averaging failed";
              return;
            }
        ...
        for (const auto& fetch : fetch_group) {
          ...
          if (bytes64 > UINT32_MAX ||
            ...
            impl_->error = impl_->mirror.error().empty()
                               ? "Vulkan vertex fetch range is invalid"
                               : impl_->mirror.error();
            return;
          }
        }
        ...
        if (refresh) {
          ...
          for (const auto& subresource : source_layout.subresources) {
            if (subresource.guest_size_bytes > UINT32_MAX ||
              ...
              impl_->error = impl_->mirror.error().empty()
                                 ? "Vulkan GPU-authored texture range is invalid"
                                 : impl_->mirror.error();
              return;
            }
          }
          ...
          if (texture_snapshot_base >= memory::kPhysicalMemorySize ||
            ...
            impl_->error = "Vulkan texture snapshot range is invalid";
            return;
          }
          ...
          if (!impl_->memory->copy_physical_range(texture_snapshot_base,
                                                   texture_snapshot)) {
            impl_->error = "Vulkan texture snapshot failed";
            return;
          }
          const auto decoded =
    ...
    if (impl_->memory && draw->vertex_shader.valid &&
      ...
      if (decoded_vertex == impl_->decoded_shaders.end() ||
        ...
        impl_->error = "Vulkan draw lacks decoded shader reflection";
        return;
      }
        ...
        if (dynamic_memexport_range) {
          ...
          for (const auto& range : memexport_ranges) {
            ...
            if (range.size_bytes &&
              ...
              impl_->error = impl_->mirror.error().empty()
                                 ? "Vulkan memexport range is invalid"
                                 : impl_->mirror.error();
              return;
            }
          }
      ...
      auto writable_variant = [&](std::uint64_t shader_hash,
        ...
        if (!compiled->succeeded) {
          impl_->error = compiled->diagnostics.empty()
                             ? "Vulkan writable guest-memory shader compilation failed"
      ...
      if (draw->source == DrawSource::Dma) {
        ...
        if (index_bytes > UINT32_MAX ||
            std::uint64_t{draw->index_buffer.physical_address} + index_bytes >
                memory::kPhysicalMemorySize) {
          impl_->error = "Vulkan index snapshot range is invalid";
          return;
        }
        ...
        if (!impl_->memory->copy_physical_range(index_snapshot_base,
                                                 index_snapshot)) {
          impl_->error = "Vulkan index snapshot failed";
          return;
        }
      }
      ...
      if (!impl_->queue.execute_async(
            ...
            const auto descriptor_set =
                impl_->resources.descriptor_set(frame_index, draw_slot);
              ...
              if (index_byte_size >
                  impl_->transient_upload_mappings[frame_index].size() -
                      std::min(aligned_offset,
                               impl_->transient_upload_mappings[frame_index].size())) {
                impl_->error =
                    "Vulkan transient upload arena exhausted by one draw batch";
            ...
            impl_->mirror.prepare_shader_access(command_buffer, memexport_writable);
            for (std::size_t i = 0; i < color_count; ++i)
              if (active_targets[i])
            ...
            if (batch.indexed) {
              vkCmdBindIndexBuffer(
                  command_buffer, impl_->transient_uploads[frame_index].buffer(),
