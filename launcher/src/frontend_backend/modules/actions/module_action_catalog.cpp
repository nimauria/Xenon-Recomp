...
QVariantList ModuleActionCatalog::actions(const QVariantMap& module,
  ...
  const auto installing = update_status == QStringLiteral("installing");
  const auto rolling_back = update_status == QStringLiteral("rolling-back");
  const auto busy = checking || downloading || installing || rolling_back;
  const auto rollback_available = update_state.value(QStringLiteral("rollbackAvailable")).toBool();
  const auto rollback_version = update_state.value(QStringLiteral("rollbackVersion")).toString().trimmed();
  ...
  if (can_install) {
    ...
    const auto update_enabled = catalog_known && package_supported && !busy;
    QString disabled_reason;
    if (!catalog_known) {
      disabled_reason = QStringLiteral("This local module is not listed in the official Xenon Modules registry.");
    } else if (!package_supported) {
      disabled_reason = QStringLiteral("The Xenon Modules registry does not define a package for this operating system and architecture.");
    } else if (busy) {
    ...
    result.append(action(QStringLiteral("checkUpdate"),
                         checking ? QStringLiteral("Checking for updates…")
                                  ...
                                  : installing ? QStringLiteral("Installing update…")
                                  : rolling_back ? QStringLiteral("Rolling back…")
                                                 : QStringLiteral("Check for updates"),
                         QStringLiteral("\u21BB"), update_enabled, true, false, disabled_reason));
  }

  if (rollback_available) {
    const auto rollback_label = rollback_version.isEmpty()
                                    ? QStringLiteral("Roll back module")
                                    : QStringLiteral("Roll back to %1").arg(rollback_version);
    result.append(action(QStringLiteral("rollbackUpdate"), rollback_label, QStringLiteral("\u21B6"),
                         !busy, true, false,
                         busy ? QStringLiteral("Wait for the current module update operation to finish.")
                              : QString{}));
  }
  ...
  result.append(action(QStringLiteral("openRepository"), QStringLiteral("Open repository"),
                       ...
                       repository.isEmpty() ? QStringLiteral("No repository is associated with this module in the Xenon Modules registry.") : QString{}));
...
QVariantList ModuleActionCatalog::pageActions() {
  return {
      action(QStringLiteral("browseCatalog"), QStringLiteral("Browse Xenon Modules registry"), QStringLiteral("\u25C7")),
      ...
      action(QStringLiteral("refreshCatalog"), QStringLiteral("Refresh Xenon Modules registry"), QStringLiteral("\u21BB")),
